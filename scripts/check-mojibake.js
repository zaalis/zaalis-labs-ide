...
const TARGETS = [
  ...
  'README.md',
  'interface',
  path.join('scripts', 'stage-rust-binaries.ps1'),
  path.join('scripts', 'stage-rust-binaries.sh'),
  path.join('native', 'README.md'),
  // Windows shell (C++ / WebView2 / Inno Setup)
  ...
  // Linux and macOS shells (Electron). Absent entries are skipped silently,
  // so the same list serves the three editions.
  path.join('native', 'electron'),
  path.join('native', 'package_electron.js'),
  path.join('native', 'build_linux.sh'),
  path.join('native', 'build_linux.bat'),
  path.join('native', 'build_linux_deb.sh'),
  path.join('native', 'build_macos_dmg.sh'),
  path.join('native', 'build_macos_installer.sh'),
  path.join('native', 'package_macos_electron.sh'),
];
...
// "\u00c2" must be followed by a windows-1252 high byte too. Mis-decoding U+0080-U+00BF
// always yields "\u00c2" + one of those characters ("\u00c2\u00a0", "\u00c2\u00b0", "\u00c2\u00ab"\u2026), never a plain letter:
// accepting any character flagged correct French such as "T\u00c2CHE" or "\u00c2ME".
const MOJIBAKE = new RegExp('(?:\\u00c3.|\\u00c2[' + CP1252_HIGH + ']|\\u00e2[' + CP1252_HIGH + ']|\\u00f0\\u0178..|\\ufffd|\\u2b26)');
