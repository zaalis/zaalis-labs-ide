...
const automationManager = new AutomationManager({
  ...
  bridgeSecret: COMPUTER_STOP_SECRET,
});
...
function resolveDataDir() {
  ...
  if (process.pkg && process.platform === 'darwin') {
...
// (nvm, pyenv, cargo, Homebrew shellenv…) is applied exactly as in Terminal.
...
app.post('/api/pick-folder', (req, res) => {
  ...
  execFile('osascript', ['-e', script], { timeout: 180000, env: execEnv() }, (err, stdout) => {
    if (err) {
      // -128 / « User canceled » = l'utilisateur a ferme la fenetre, pas une erreur.
...
// Ramene une variante demandee (configuration utilisateur, ou heritee d'une
// autre plateforme) sur une variante que ce Mac sait reellement lancer.
function normalizeEngineVariant(variant) {
  const v = String(variant || '').toLowerCase();
  if (v === 'metal' || v === 'cpu') return v;
  if (v === 'cuda' || v === 'vulkan' || v === 'rocm') return 'metal';
  return detectEngineVariant();
}

// Apple Silicon (arm64) et Intel (x64) ont chacun leur binaire. process.arch
// vaut celui du serveur empaquete, donc il designe bien la tranche a utiliser,
// y compris sous Rosetta.
...
function engineAssetUrls(variant) {
  ...
  // Le binaire macOS embarque Metal ; le mode CPU est impose au lancement
  // avec -ngl 0 plutot qu'avec un telechargement different.
  return [base + `llama-${LLAMA_TAG}-bin-macos-${macosEngineArch()}.tar.gz`];
}

// Les archives tar ne conservent pas toujours le bit d'execution selon l'outil
// d'extraction, et Gatekeeper refuse un binaire non executable.
...
app.get('/api/check-update', async (req, res) => {
  try {
    ...
    // Une release publie les paquets des trois plateformes : on prend le .dmg de
    // l'architecture de ce Mac (arm64 pour Apple Silicon, x64 pour Intel), avec
    // un repli sur le premier .dmg disponible si le nommage change.
    const assets = release.assets || [];
    const wanted = process.arch === 'arm64' ? 'arm64' : 'x64';
    const asset = assets.find((a) => /\.dmg$/i.test(a.name) && a.name.includes(wanted))
...
app.post('/api/update/download', (req, res) => {
  try {
    ...
    const dest = path.join(fs.existsSync(downloadsDir) ? downloadsDir : os.tmpdir(), 'zaalis-update.dmg');
    downloadProgress = 0;
...
app.post('/api/update/install', (req, res) => {
  try {
    const installerPath = downloadedInstallerPath || path.join(os.homedir(), 'Downloads', 'zaalis-update.dmg');
    if (!fs.existsSync(installerPath)) {
      return res.status(409).json({ error: 'Installer not downloaded yet.' });
    }

    // On demande au Finder de monter l'image disque : c'est lui qui detient le
    // processus, donc le montage survit a la fermeture de l'IDE. L'utilisateur
    // glisse ensuite l'application dans /Applications, comme pour toute app
    // macOS — remplacer un bundle signe pendant qu'il tourne le casserait.
    ...
    res.json({ success: true, installerPath, manual: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
async function startOllamaIfNeeded() {
  ...
  // Ollama.app embarque son binaire ; Homebrew l'installe dans /opt/homebrew
  // (Apple Silicon) ou /usr/local (Intel). On garde `ollama` du PATH en dernier
  // recours, le gestionnaire 'error' rendant une absence inoffensive.
  const candidates = [
